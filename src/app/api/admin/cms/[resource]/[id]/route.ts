...
export async function PUT(req: Request, { params }: Params) {
  ...
  return handle(req, resourcePermission(resource), async (s) => ({ row: await updateRow(resource, id, await json(req), s.actor) }));
}

export async function DELETE(req: Request, { params }: Params) {
  const { resource, id } = await params;
  const reassignTo = new URL(req.url).searchParams.get("reassign_to");
  return handle(req, resourcePermission(resource), (s) => deleteRow(resource, id, s.actor, { reassignTo }));
}
