...
export function EventRow({ event, headingLevel = "h3" }: { event: EventItem; headingLevel?: "h2" | "h3" }) {
    ...
    <article id={event.slug} className="scroll-mt-28 border-b border-line">
      ...
      >
        <div className="flex items-baseline gap-3 sm:block">
          <div className="text-4xl font-semibold leading-none tracking-[-0.04em] tnum">{eventDay(event.starts_at)}</div>
        ...
        <div>
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
            <span
              className={cn(
                ...
                event.status === "ongoing" ? "text-acm-bright" : event.status === "cancelled" ? "text-ink-ghost" : "text-ink-faint",
              )}
            >
              {STATUS_LABEL[event.status]}
            </span>
            <span className="h-3 w-px bg-line-strong" aria-hidden />
            <span className="font-mono text-micro uppercase text-ink-faint">{eventWhen(event.starts_at, event.ends_at)}</span>
          </div>

          <Heading
            className={cn(
              "mt-4 text-2xl font-semibold tracking-[-0.03em] text-balance",
              event.status === "cancelled" && "text-ink-faint line-through decoration-line-strong",
            )}
          >
            {event.title}
          </Heading>

          {event.location ? <p className="mt-2 text-sm text-ink-faint">{event.location}</p> : null}
          {event.description ? (
          ...
          {open && event.registration_url ? (
            <a
              ...
              className="group mt-6 inline-flex h-11 items-center gap-3 bg-acm-solid px-6 font-mono text-label uppercase text-white transition-colors duration-200 hover:bg-acm-deep"
            >
              Register
        ...
        {event.image_url ? (
          ...
          <img
            ...
            loading="lazy"
            decoding="async"
            className="aspect-[4/3] w-full border border-line object-cover sm:col-start-2 lg:col-start-auto"
          />
        ) : null}
      </div>
    </article>
