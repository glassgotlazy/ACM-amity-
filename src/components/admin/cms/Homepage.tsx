...
const SECTIONS: Record<SectionKey, SectionUi> = {
  hero: {
    name: "Hero",
    about:
      "The top of the homepage. Always shown. Leave any field empty to hide that element.",
    fields: [
      "eyebrow",
      "title",
      "subtitle",
    ...
    hints: {
      subtitle: "Short lines beside the description; one per line.",
      image: "Replaces the pipeline graphic when set.",
    },
  },
  announcements: {
    name: "Announcements",
    about:
      "A notice strip under the hero. Items are managed under Announcements; hidden when none are published.",
    fields: ["eyebrow"],
  ...
  what_we_build: {
    ...
    labels: {
      body: "Description",
      subtitle: "Small print under the description",
      note: "Note box",
      primary: "Link",
    },
  },
  problem_lab: {
    name: "Problem Lab",
    about:
      "Introduces the problem statements. The last headline line is set in the accent colour.",
  ...
  events: {
    name: "Events",
    about:
      "The next three events. Hidden automatically when nothing is coming up.",
  ...
  ideas: {
    name: "Project ideas",
    about: "The first six project ideas.",
    fields: ["eyebrow", "title", "body", "primary"],
    labels: { primary: "Link" },
  },
  contribution: {
  ...
  recruit: {
    name: "Recruit",
    about:
      "Call to action. Also shown on the Projects and Project Ideas pages. The last headline line is set fainter.",
    fields: ["title", "body", "primary", "secondary"],
  },
  final_cta: {
    name: "Closing statement",
    about:
      "The end of the homepage. The words beneath it are the Pillars in Site Settings.",
    fields: ["title", "subtitle", "body"],
    labels: { title: "First headline", subtitle: "Second headline" },
    hints: {
      subtitle: `${HEADLINE_HINT} The last line is set in the accent colour.`,
    },
  },
};
...
export function HomepageEditor() {
  ...
  async function put(row: Row, overwrite = false) {
    return api<{ row: Row }>(`/api/admin/cms/sections/${row.key}`, {
      ...
      json: {
        ...row,
        _expected_updated_at: overwrite ? undefined : row.updated_at,
      },
    });
  }
  ...
  async function save(overwrite = false) {
    ...
    try {
      ...
      if (e instanceof ApiError && e.code === "stale")
        setStale(e.extra.current as Row);
  ...
  async function flip(row: Row) {
    ...
    try {
      ...
      setRows(
        (rs) =>
          rs?.map((r) => (r.key === row.key ? { ...r, ...saved } : r)) ?? null,
      );
      toast(
        "ok",
        `${SECTIONS[row.key].name} is now ${row.enabled ? "hidden" : "shown"}.`,
      );
    } catch (e) {
  ...
  async function move(index: number, delta: -1 | 1) {
    ...
    setRows(next);
    try {
      await api("/api/admin/cms/sections/reorder", {
        method: "POST",
        json: { keys: next.map((r) => r.key) },
      });
    } catch (e) {
      setRows(previous);
      toast("error", explain(e));
        ...
        {rows.map((row, i) => {
          ...
          return (
            <motion.li
              ...
              layout="position"
              transition={snap}
              className="relative flex flex-wrap items-center gap-x-4 gap-y-2 border-b border-line bg-void px-3 py-3 last:border-b-0"
            >
              <span className="w-16">
                {pinned ? (
                  <span className="px-2 font-mono text-micro uppercase text-ink-ghost">
                    First
                  </span>
                ) : (
                  <MoveButtons
        ...
        {editing ? (
          ...
          >
            ...
            <p className="text-sm text-ink-muted">
              {SECTIONS[editing.key].about}
            </p>
            <SectionFields
              ui={SECTIONS[editing.key]}
              value={editing}
              set={(p) => setEditing((r) => (r ? ({ ...r, ...p } as Row) : r))}
              errors={errors}
            />
          </form>
        ) : null}
...
function SectionFields({
  ...
  const setExtra = (p: Record<string, unknown>) =>
    set({ extra: { ...extra, ...p } });
  ...
  return (
    <>
      {has("eyebrow") ? (
        <TextInput
          label={label("eyebrow", "Eyebrow")}
          hint="Small label above the headline."
          value={s(value.eyebrow)}
      ...
      {has("hero_extra") ? (
        <TextInput
          label="Badge"
          hint="Small text after the eyebrow."
          value={s(extra.badge)}
          onChange={(v) => setExtra({ badge: v })}
          error={errors["extra.badge"]}
          maxLength={60}
        />
      ) : null}
      {has("title") ? (
        <TextArea
          ...
          hint={ui.hints?.title ?? HEADLINE_HINT}
          rows={3}
          value={s(value.title)}
      ...
      {has("subtitle") ? (
        <TextArea
          ...
          hint={ui.hints?.subtitle}
          rows={2}
          value={s(value.subtitle)}
      ...
      {has("primary_label") ? (
        <TextInput
          label={label("primary_label", "Button text")}
          value={s(value.primary_label)}
          onChange={(v) => set({ primary_label: v })}
          error={errors.primary_label}
          maxLength={40}
        />
      ) : null}
      {has("primary") ? (
        <div className="grid gap-4 sm:grid-cols-2">
          <TextInput
            label={`${label("primary", "Primary button")} text`}
            value={s(value.primary_label)}
            onChange={(v) => set({ primary_label: v })}
            error={errors.primary_label}
            maxLength={40}
          />
          <LinkInput
            label={`${label("primary", "Primary button")} link`}
            value={s(value.primary_href)}
            onChange={(v) => set({ primary_href: v })}
            error={errors.primary_href}
          />
        </div>
      ) : null}
      {has("secondary") ? (
        <div className="grid gap-4 sm:grid-cols-2">
          <TextInput
            ...
            value={s(value.secondary_label)}
            onChange={(v) => set({ secondary_label: v })}
            error={errors.secondary_label}
            maxLength={40}
          />
          <LinkInput
            label="Secondary button link"
            value={s(value.secondary_href)}
            onChange={(v) => set({ secondary_href: v })}
            error={errors.secondary_href}
          />
        </div>
      ) : null}
      {has("hero_extra") ? (
        <>
          <div className="grid gap-4 sm:grid-cols-2">
            <TextInput
              label="Text link"
              hint="The underlined third link."
              value={s(extra.tertiary_label)}
              onChange={(v) => setExtra({ tertiary_label: v })}
              error={errors["extra.tertiary_label"]}
              maxLength={60}
            />
            <LinkInput
              label="Text link destination"
              value={s(extra.tertiary_href)}
              onChange={(v) => setExtra({ tertiary_href: v })}
              error={errors["extra.tertiary_href"]}
            />
          </div>
          <ListField
            label="Focus areas"
            hint="The strip along the bottom of the hero. Up to 8 read best."
            items={list(extra.focus)}
            onChange={(v) => setExtra({ focus: v })}
            error={errors["extra.focus"]}
            max={12}
          />
        </>
      ) : null}
      {has("image") ? (
        <ImageField
          ...
          use="cover"
          value={(value.image_url as string) ?? null}
...
function StaleNotice({
  ...
  return (
    ...
    >
      ...
      <p className="mt-1">
        Saving now would overwrite their changes. Choose which version to keep.
      </p>
...
const PAGES: Record<
  string,
  { name: string; path: string; note?: string; accent?: boolean }
> = {
  ...
  page_activity: {
    name: "Activity",
    path: "/activity",
    note: "Small print under the header",
  },
...
export function PagesEditor() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setRows(
        (await api<{ rows: Row[] }>("/api/admin/cms/sections?pages=1")).rows,
      );
    } catch (e) {
      setLoadError(e);
    }
  ...
  async function save(overwrite = false) {
    ...
    try {
      await api(`/api/admin/cms/sections/${editing.key}`, {
        ...
        json: {
          ...editing,
          _expected_updated_at: overwrite ? undefined : editing.updated_at,
        },
      });
          ...
          return (
            ...
            >
              <span className="min-w-0 flex-1">
                ...
                <span className="block truncate text-xs text-ink-faint">
                  <code className="font-mono">{meta.path}</code> ·{" "}
                  {s(row.title).split("\n")[0]}
                </span>
              </span>
              <Btn
        ...
        {editing ? (
          ...
          >
            ...
            <TextArea
              ...
              hint={
                ui?.accent
                  ? `${HEADLINE_HINT} The last line is set in the accent colour.`
                  : HEADLINE_HINT
              }
              rows={3}
