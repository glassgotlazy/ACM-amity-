...
import { Btn, ConfirmDialog, Drawer, EmptyState, ErrorState, LoadingRows, MoveButtons, Pill, useCms, useToast } from "./kit";

type Row = { id: string } & Record<string, unknown>;
export type Draft = Record<string, unknown>;
export type Errors = Record<string, string>;

export type FormProps = { value: Draft; set: (patch: Draft) => void; errors: Errors };

export type CollectionProps = {
  resource: string;
  /** Singular noun for buttons and messages: "event", "team member". */
  noun: string;
  columns: { head: string; cell: (row: Row) => ReactNode; className?: string }[];
...
export function Collection(props: CollectionProps) {
  const { resource, noun, columns, searchText, blank, Form, label, sortable, toggle } = props;
  ...
  async function move(index: number, delta: -1 | 1) {
    ...
    setRows(next);
    try {
      await api(`/api/admin/cms/${resource}/reorder`, { method: "POST", json: { ids: next.map((r) => r.id) } });
    } catch (e) {
      setRows(previous);
      toast("error", explain(e));
    }
  ...
  async function remove() {
    ...
    setDeleting(true);
    try {
      await api(`/api/admin/cms/${resource}/${doomed.id}`, { method: "DELETE" });
  ...
  return (
    <div>
      ...
      <div className="mt-5">
        {loadError ? (
          ...
          <EmptyState
            title={`No ${noun}s yet`}
            action={
              <Btn tone="primary" onClick={() => setEditing({ id: null, draft: blank() })}>
                + New {noun}
              </Btn>
            }
          >
          ...
          <div className="relative overflow-x-auto border border-line">
            <table className="w-full min-w-[40rem] text-left text-sm">
              <thead className="border-b border-line bg-surface">
                <tr>
                  {reorderable ? (
                    <th className="w-20 px-3 py-3 font-mono text-micro font-normal uppercase text-ink-faint">Order</th>
                  ) : null}
                  {columns.map((c) => (
                    <th key={c.head} className={cn("px-3 py-3 font-mono text-micro font-normal uppercase text-ink-faint", c.className)}>
                      {c.head}
                    </th>
                  ))}
                  {toggle ? <th className="px-3 py-3 font-mono text-micro font-normal uppercase text-ink-faint">Status</th> : null}
                  ...
                  <tr key={row.id} className="border-b border-line last:border-b-0 hover:bg-surface/60">
                    {reorderable ? (
                      <td className="px-1 py-2">
                        <MoveButtons
                          ...
                          onDown={i < shown.length - 1 ? () => move(i, 1) : undefined}
                        />
                      </td>
                    ) : null}
                    {columns.map((c) => (
                      <td key={c.head} className={cn("px-3 py-3 align-middle text-ink-muted", c.className)}>
                        {c.cell(row)}
                      </td>
                    ))}
                    {toggle ? (
                      <td className="px-3 py-3">
                        ...
                        >
                          <Pill on={Boolean(row[toggle.key])}>{row[toggle.key] ? toggle.on : toggle.off}</Pill>
                        </button>
                      </td>
                    ) : null}
                    <td className="whitespace-nowrap px-3 py-2 text-right">
                      ...
                      <Btn
                        ...
                        onClick={() => (props.onDelete ? props.onDelete(row, load) : setDoomed(row))}
        ...
        footer={
          <>
            {Object.keys(errors).length ? (
              <span role="alert" className="mr-auto text-xs font-medium text-acm-bright">
      ...
      >
        {editing ? (
          ...
          >
            ...
            {Form({
              value: editing.draft,
              set: (patch) => setEditing((ed) => (ed ? { ...ed, draft: { ...ed.draft, ...patch } } : ed)),
              errors,
            })}
      ...
      >
        {doomed && props.deleteWarning ? props.deleteWarning(doomed) : "It disappears from the public site straight away. This cannot be undone."}
      </ConfirmDialog>
    </div>
  );
}
